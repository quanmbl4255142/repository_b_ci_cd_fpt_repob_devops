# Django-app - Kubernetes Manifests
...
- **Docker Image**: ghcr.io/QuanMBL4255142/my-django-app1
- **Port**: 8000
...
# Copy manifests
cp <this-directory>/*.yaml apps/Django-app/
cp argocd-application.yaml argocd-apps/Django-app-app.yaml

# Commit and push
git add apps/Django-app/ argocd-apps/Django-app-app.yaml
git commit -m "Add Django-app application"
...
kubectl apply -f argocd-apps/Django-app-app.yaml

# Check status
kubectl get applications -n argocd-new

# Watch sync
kubectl get app Django-app-app -n argocd-new -w
```
...
3. Image pushed to registry: ghcr.io/QuanMBL4255142/my-django-app1
...
### Force sync
```bash
kubectl patch app Django-app-app -n argocd-new \
...
### Scale replicas
```bash
kubectl scale deployment/Django-app -n Django-app --replicas=3
```

### Delete application
```bash
kubectl delete app Django-app-app -n argocd-new
